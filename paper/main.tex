...
(2)~A nine-experiment mechanistic study that adjudicates among perception, modality, and generation-side accounts, ruling out the first two---including behavioral modality probes on the frontier models themselves---and supporting \textbf{perceive--externalize--synthesize} (\S\ref{sec:mechanism}).
...
Three hypotheses are consistent with \S\ref{sec:observation} but make different predictions. \textbf{The modality hypothesis:} the model reasons better over text tokens than over raw audio/image tokens. \textbf{The generation-load hypothesis:} a single pass cannot simultaneously perceive, reason, and write a long faithful review, so it satisfices; each step needs a dedicated, full-budget pass. \textbf{The attention-dilution hypothesis:} raw-modality tokens are numerous and low-density, diluting attention. We ran nine experiments to separate them, reusing the probe/hallucination judge of \S\ref{sec:observation} throughout; all artifacts are released (\S\ref{appdx:code}). The verdict: the modality hypothesis fails every test we can run, the generation-load hypothesis is supported by the two load-bearing contrasts (both multi-seed on all 21 cells), and the attention-dilution hypothesis is untestable on Gemini with our tools (its image pipeline normalizes DPI away---the DPI control). Details and per-experiment tables are in Appendix~\ref{appdx:substrate}.
...
\end{subfigure}
\caption{\textbf{Text is not a better reasoning medium than pixels (the modality hypothesis is false for legible content).}
...
The evidence converges on the generation-load hypothesis. The model perceives the content (the perception check); the modality is not the reasoning bottleneck for legible content, at the activation level and behaviorally on the frontier models (the modality probes); and recovery requires two passes with independent budgets (the single-call test), not merely text in context (the modality-added control). One pass asked to do everything satisfices under generation load; a dedicated externalization pass followed by a dedicated, full-budget synthesis pass over complete text does not. This account makes two predictions we test next: the benefit should scale with how much the end-to-end pass satisfices (an \emph{inverse-baseline law}, \S\ref{sec:solution}), and decomposition should fail in exactly two ways---when synthesis cannot fit the transcript through its output budget, and when removing the grounding source lets synthesis confabulate from training priors (\S\ref{sec:failure_modes}).
