...
Three hypotheses are consistent with \S\ref{sec:observation} but make different predictions. \textbf{The substrate hypothesis:} the model reasons better over text tokens than over raw audio/image tokens. \textbf{The generation-load hypothesis:} a single pass cannot simultaneously perceive, reason, and write a long faithful review, so it satisfices; each step needs a dedicated, full-budget pass. \textbf{The attention-dilution hypothesis:} raw-modality tokens are numerous and low-density, diluting attention. We ran nine experiments to separate them, reusing the probe/hallucination judge of \S\ref{sec:observation} throughout; all artifacts are released (\S\ref{appdx:code}). The verdict: the substrate hypothesis fails every test we can run, the generation-load hypothesis is supported by the two load-bearing contrasts (both multi-seed on all 21 cells), and the attention-dilution hypothesis is untestable on Gemini with our tools (its image pipeline normalizes DPI away---the DPI control). Details and per-experiment tables are in Appendix~\ref{appdx:substrate}.
...
\end{subfigure}
\caption{\textbf{Text is not a better reasoning substrate than pixels (the substrate hypothesis is false for legible content).}
\textbf{(a)}~Vision activation probe: novel synthetic facts presented as text vs.\ as an \emph{image} of that text are read with the same accuracy and the same answer-token log-prob at every load $K\le 24$; readout depth is TOST-equivalent within $\pm1$ layer (replicated on Qwen2.5-VL-3B; Appendix~\ref{appdx:vision_ladder}). An omni-model audio analogue (the audio activation probe) gives the same null once the TTS is intelligible (Appendix~\ref{appdx:substrate}).
\textbf{(b)}~The null holds behaviorally on the exact headline models. Frontier retrieval probe (left, center): single-fact retrieval from packed sets of up to $K{=}48$ novel facts is \emph{identical} from text and legible image on both models (pooled gap $+0.000$, TOST-equivalent within $\pm0.1$), while a degraded low-DPI control collapses to $0.15$--$0.27$---the probe detects gaps when they exist. Frontier reasoning probe (right): multi-hop reasoning (sum 6 packed facts; the answer is never visible). Gemini is at ceiling in both modalities; Claude without extended thinking has headroom (text $0.825$) and the image is reliably \emph{better} ($1.00$, gap $+0.175$, 95\% CI $[0.10,0.26]$)---the opposite of the substrate hypothesis's prediction.}
\label{fig:substrate}
\end{figure}

\paragraph{Not substrate.}
The intuitive explanation---text is intrinsically easier to reason over than pixels or audio---is directly contradicted at two levels (Figure~\ref{fig:substrate}). At the \emph{activation} level, a layer-wise logit-lens on two open VLMs reads novel synthetic facts equally well from text and from an image of that text, for one fact and for up to 24 packed facts (vision activation probe); an omni-model analogue finds the same for speech once the TTS is intelligible---an apparent audio gap with robotic TTS vanishes with two independent clean neural voices, so audio-substrate claims must control for intelligibility (audio activation probe). \emph{Behaviorally on the headline models}, retrieval from pixels equals retrieval from text at every load up to 48 packed facts, and a degraded-image positive control confirms the probes are sensitive (frontier retrieval probe). Forcing multi-hop \emph{reasoning} (frontier reasoning probe) sharpens the contradiction: on the one model with headroom, the image is reliably better than text, not worse. The frontier probes are behavioral, not activation-level, and Gemini's reasoning probe sits at ceiling, so Claude supplies the single headroom case; with those caveats, there is no ``pixels are harder to reason over'' penalty for legible propositional content on the capable VLMs we test---a scope the modality-gap literature both supports and bounds (\S\ref{sec:related}). The contrast with \S\ref{sec:observation} is the point: single-fact retrieval from pixels is \emph{perfect} on these same models, yet their end-to-end reviews drop a third of the content. The loss is not in reading the modality.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/fig_e2_collapse.pdf}
\caption{\textbf{Single-call test: the win needs two passes with independent output budgets; modality-added control: removing the modality is not the engine.} Probe coverage on the 8-cell condition grid (single-seed; raw counts in Appendix~\ref{appdx:substrate}, Table~\ref{tab:mech_cond}). A single call instructed to transcribe \emph{then} review (red squares) collapses below the two-pass configuration on every paper that produced a review except Perovskite ($0.80$ vs.\ $0.74$), and on three cells emits no review at all---the transcript exhausts the output budget. At $n{=}5$ seeds on all 21 cells the collapse replicates: single-call coverage sits below two-pass by $\Delta_{\rm cov}{=}-0.111$ (bootstrap 95\% CI $[-0.170,-0.038]$, Wilcoxon $p{=}0.019$, mixed-effects $p{<}10^{-3}$), with 7 runs emitting no review. Conversely, giving the synthesis pass the transcript \emph{and} the raw modality ($C_{1+}$, open diamonds) preserves and sometimes maximizes coverage; at $n{=}5$ on all 21 cells it shifts coverage by only $+0.069$ (CI $[-0.001,+0.154]$, inconclusive) while re-adding hallucinations on 14/21 cells---the Mode-B effect of \S\ref{sec:failure_modes}.}
\label{fig:e2_collapse}
\end{figure}

\paragraph{It is generation under load.}
If externalizing content into context were sufficient, one call that first transcribes verbatim and then writes the review---everything in a single context window---should match the two-pass decomposition. It does not (Figure~\ref{fig:e2_collapse}): the single call collapses, either compressing the review far below the two-pass level or exhausting its entire output budget on the transcript and emitting no review at all. The complementary control (the modality-added control) shows the flip side: giving the synthesis pass \emph{both} the transcript and the raw modality does not break the benefit---pixels in context are not harmful to coverage, consistent with the vision activation probe---but their presence re-introduces some hallucinations relative to pure-text synthesis. So the active ingredient is the \emph{decomposition into two passes with independent output budgets}; removing the modality buys faithfulness only at the margin. Two further controls did not separate the hypotheses and are reported with their confounds in Appendix~\ref{appdx:substrate}: DPI cannot move Gemini's vision-token count (the DPI control), and \texttt{pdftotext} scrambles two-column reading order (the reading-order control).

\paragraph{Verdict: perceive, externalize, synthesize.}
The evidence converges on the generation-load hypothesis. The model perceives the content (the perception check); the modality is not the reasoning bottleneck for legible content, at the activation level and behaviorally on the frontier models (the substrate probes); and recovery requires two passes with independent budgets (the single-call test), not merely text in context (the modality-added control). One pass asked to do everything satisfices under generation load; a dedicated externalization pass followed by a dedicated, full-budget synthesis pass over complete text does not. This account makes two predictions we test next: the benefit should scale with how much the end-to-end pass satisfices (an \emph{inverse-baseline law}, \S\ref{sec:solution}), and decomposition should fail in exactly two ways---when synthesis cannot fit the transcript through its output budget, and when removing the grounding source lets synthesis confabulate from training priors (\S\ref{sec:failure_modes}).
...
\emph{Mechanism breadth:} all measurements are on long-form review---we claim nothing about short-form QA, code, or agentic tasks. the frontier probes are behavioral, not activation-level (frontier activations are inaccessible); Gemini's reasoning probe sits at ceiling, so Claude supplies the one headroom case; real scanned/degraded pages remain untested (the degraded control is synthetic); the DPI and reading-order controls are single-seed and confounded.
...
This appendix expands the mechanism sections (the perception check of \S\ref{sec:observation}, and the decomposition controls, substrate probes, and confounded controls of \S\ref{sec:mechanism}) with the per-experiment tables.
...
\begin{table}[h]
\small\centering
\caption{Audio activation probe readout (Qwen2.5-Omni Thinker, 24 novel facts): the apparent audio substrate gap is a TTS-intelligibility artifact. ``Intellig.'': fraction of clips the model's own ASR transcribes correctly. Depth-gap = audio$-$text readout depth (layers). TOST SESOI $\pm 1$ layer.}
\label{tab:e7}
...
\begin{table}[h]
\small\centering
\caption{Frontier retrieval probe: behavioral cross-modal substrate test ($T{=}20$ seeded trials per $K$, single-number retrieval). ``legible img'': image of the same text at $34$px. ``degraded img'': small-font, downscaled+blurred low-DPI-scan control. Gap = image$-$text accuracy; TOST SESOI $\pm 0.1$.}
\label{tab:e8}
\setlength{\tabcolsep}{4pt}
...
\begin{sloppypar}
All code, prompts, per-cell judgments, and the mechanistic-experiment artifacts (including the frontier substrate probes, the running-example run directory, and the within-Claude cross-vendor comparison) are released in the project repository, whose README documents the pipelines and the exact commands to reproduce every cell, table, and figure in this paper.
\end{sloppypar}
