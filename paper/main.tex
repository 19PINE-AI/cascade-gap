...
\textbf{The observation (\S\ref{sec:observation}).} When a frontier model reviews a long source end-to-end, it quietly satisfices. On our running example---Andrej Karpathy's 42-minute \emph{State of GPT} talk---Gemini~3.1~Pro's end-to-end review covers barely six in ten of the atomic facts an independent judge extracts from the source, and a quarter of its claims are unsupported (Figure~\ref{fig:example}). The failure is not perception: asked to simply \emph{transcribe} the same audio, the same weights produce a transcript containing every single fact the review dropped. Across all 21 cells, $99.7\%$ of the content the end-to-end pass drops sits verbatim in the model's own transcript (Figure~\ref{fig:mech_e3}). The model perceives the content and then loses it downstream.
...
\textbf{(c)}~Gemini 2.5 Flash is a \emph{strong} end-to-end audio reader (its $C_0$ beats Pro's on 4/5 cells), so its cascade gains appear only on the two headroom cells (MIT, Harvard) and Karpathy regresses---headroom-gated, as the law predicts.
...
\caption{Inter-judge replication, all 21 cells.}
\end{subfigure}\hfill
