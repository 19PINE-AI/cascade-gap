...
Two-Pass Decomposition for Long-Form Multimodal Review}
...
\date{}
\runningtitle{Transcribe, Then Reason: Two-Pass Decomposition for Long-Form Multimodal Review}
...
% =================================================================
\begin{abstract}
The default way to deploy a multimodal model on long-form review is end-to-end: feed it the raw audio or page images and ask for the review in one call. We show this default quietly \emph{satisfices}: across 21 review cells, the end-to-end pass drops roughly a third of the source content and embellishes what it keeps---despite perceiving it, since $99.7\%$ of the dropped content is captured in the same model's own transcript of the same source. A nine-experiment mechanistic study shows the bottleneck is neither perception nor the reasoning substrate---identical propositional content is read equally well from text and from an \emph{image} of that text---but \textbf{generation under load}: one pass cannot perceive, reason, and write a long faithful review at once. The mechanism dictates the fix: two same-weights passes---transcribe, then review the transcript---each with its own full output budget. This decomposition beats end-to-end on faithfulness (18/21 cells) and coverage (16/21), both sign-test significant and stable under $n{=}5$ reruns, the gain scaling inversely with the end-to-end baseline ($r{=}-0.45$): the more the monolithic pass satisfices, the more decomposition recovers. The same headroom law organizes further vendors and mixed-model pipelines---where decomposition even unlocks text-only frontier models for audio review---and predicts two failure modes, both observed and partially repaired: synthesis-budget overflow on very long sources, and training-prior confabulation once the grounding source is removed. The win is the decomposition, not the modality.
\end{abstract}
\vspace{-0.4em}
\begin{center}
\footnotesize
...
\vspace{-8pt}
\caption{\textbf{Decomposing long-form review into transcribe-then-review beats the end-to-end call, across models and pipelines.} Each dot is one review cell: the change in \textbf{(a)} probe coverage and \textbf{(b)} unsupported-claim count from end-to-end ($C_0$) to two-pass decomposition ($C_1$), same weights per row (\texttt{gpt-audio} rows are mixed pipelines vs.\ the native audio model's end-to-end pass). Coverage improves on 16/21 cells and hallucinations fall on 18/21 (sign tests $p{=}0.027$, $p{=}0.0015$).}
\label{fig:headline}
\end{figure}
