...
if e3:
    ...
    ax.bar(x, c1, 0.25, label="Two-pass review (reason over text)", color="#7fcdbb")
    ax.bar(x+0.25, c0, 0.25, label="End-to-end review (reason over modality)", color="#d95f0e")
...
if present:
    ...
    ax.bar(x-2*w,[b(c).get("C0",{}).get("coverage") or np.nan for c in present],w,label="End-to-end (image/audio)",color="#d95f0e")
    ax.bar(x-w,[b(c).get("C1",{}).get("coverage") or np.nan for c in present],w,label="Two-pass (text)",color="#7fcdbb")
    ax.bar(x,[ (e1m.get(c) or {}).get("coverage", np.nan) for c in present],w,label="Transcript+source (both)",color="#2c7fb8")
