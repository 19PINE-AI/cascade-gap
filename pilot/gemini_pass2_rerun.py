"""Re-run Gemini Pass-2 (text-only, temp=0.0) on the same prompt to verify determinism.

Usage:
  python3 pilot/gemini_pass2_rerun.py \
      --run-dir runs/paper-review-19700025120-1777462513 \
      --transcript-file transcript_C1_pass1.txt \
      --label C1_gemini_rerun1
"""
...
from pilot_paper_review import pass2_review_prompt, call_gemini_text_only
...
def main():
    ...
    if review_path.exists():
        ...
        review, ms = call_gemini_text_only(pass2_review_prompt(transcript), args.model)
